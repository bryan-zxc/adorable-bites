---
name: github-board
description: Manage GitHub project board tickets for Adorable Bites. Use whenever creating issues — epics, stories, or bugs — transitioning ticket status, completing tickets, or any project board management. Triggers on requests to create tickets, track work, manage the backlog, or update issue status.
---
...
- **Project number:** 12
- **Project node ID:** PVT_kwHOCz6Fr84BRu3E
- **Repo:** bryan-zxc/adorable-bites
- **Status field ID:** PVTSSF_lAHOCz6Fr84BRu3Ezg_eUtE
...
After creating any issue, add it to the board **and immediately set its status to Todo**. Items added via CLI have no status by default — without this step they won't appear in any board column.

```bash
ITEM_ID=$(gh project item-add 12 --owner @me --url <issue-url> --format json --jq '.id')

gh project item-edit \
  --project-id PVT_kwHOCz6Fr84BRu3E \
  --id "$ITEM_ID" \
  --field-id PVTSSF_lAHOCz6Fr84BRu3Ezg_eUtE \
...
```bash
gh issue create --repo bryan-zxc/adorable-bites \
  --title "Epic title" \
  --body "Description" \
  --label "epic" \
  --assignee "@me"
...
```bash
EPIC_NODE_ID=$(gh api repos/bryan-zxc/adorable-bites/issues/<epic-number> --jq '.node_id')
STORY_NODE_ID=$(gh api repos/bryan-zxc/adorable-bites/issues/<story-number> --jq '.node_id')
...
```bash
ITEM_ID=$(gh project item-list 12 --owner @me --limit 500 --format json --jq ".items[] | select(.content.number == <issue-number>) | .id")
...
- **Never use `gh issue close` or `gh issue reopen`** — manage completion exclusively through board status transitions. Issue state (open/closed) and board status (Todo/In Progress/Done) are separate concepts; we only use board status.
- Always assign `@me` — this dynamically resolves to whoever is running the command
- Always add issues to project board 12 after creation
